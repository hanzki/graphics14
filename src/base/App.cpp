...
vector<Vertex> loadUserGeneratedModel() {
	...
	for(auto i = 0u; i < faces; ++i)	{
		v0.position = Vec3f(FW::cos(angle_increment * i) * radius, -1.0f, FW::sin(angle_increment * i) * radius);
		v1.position = Vec3f(0.0f);
		v2.position = Vec3f(FW::cos(angle_increment * (i + 1)) * radius, -1.0f, FW::sin(angle_increment * (i + 1)) * radius);

		v0.normal = v1.normal = v2.normal = FW::cross(v1.position - v0.position, v2.position - v0.position).normalized();
		// YOUR CODE HERE (R2)
